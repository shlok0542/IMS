...
export default function EditInventory() {
  ...
  const loadItem = async () => {
    ...
    try {
      ...
      setForm({
        ...
        dateAdded: data.dateAdded ? new Date(data.dateAdded).toISOString().slice(0, 10) : ""
      });
    } catch (err) {
  ...
  if (loading) {
    return <div className="card p-6 text-slate-600">Loading item...</div>;
  }

  if (!form) {
    return <div className="card p-6 text-slate-600">Item not found.</div>;
  }

  return (
    <div className="mx-auto max-w-2xl">
      <div className="card p-6">
        <h2 className="text-2xl font-semibold">Edit Inventory</h2>
